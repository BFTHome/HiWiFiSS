...
FILENAME=ss-redir
echo ">>>正在关闭插件..."
/etc/init.d/tonycn stop
echo ">>>正在下载$NAME..."
curl -k -o `which $FILENAME` $DOWNFROM/$VERSION/$FILENAME
echo ">>>正在下载$NAME..."
chmod +x `which $FILENAME`
echo ">>>正在替换管理界面选项..."
cat /usr/lib/lua/luci/view/admin_web/tonycn/ss_setup_node.htm|sed '1,/ss_server_meth/!d' > /tmp/3.txt
echo '<option value="">请选择加密方式</option>' >> /tmp/3.txt
...
cat /usr/lib/lua/luci/view/admin_web/tonycn/ss_setup_node.htm|sed '/ <\/select/,$!d' >> /tmp/3.txt
mv /tmp/3.txt /usr/lib/lua/luci/view/admin_web/tonycn/ss_setup_node.htm
