...
if [ $? -eq 0 ]; then
    if [ $(grep 'admin_web.*upgrade' /usr/lib/lua/luci/view/admin_web/home.htm|wc -l) -eq 1 ]; then
        ...
        if [ $(grep $NAME /usr/lib/lua/luci/view/admin_web/home.htm|wc -l) -lt 1 ]; then
            echo ">>>修改首页按钮..."
            sed -i 's/admin_web.*upgrade.*%>/admin_web'"'"','"'"'tonycn'"'"')%>">'"$NAME"'/' /usr/lib/lua/luci/view/admin_web/home.htm
        fi
    else
        ENTRY="互联网页"
        ...
        if [ $(grep $NAME /usr/lib/lua/luci/view/admin_web/network/index.htm|wc -l) -lt 1 ]; then
            echo ">>>在互联网页插入按钮..."
            sed -i 's/<ul id="advanced_setup">/<ul id="advanced_setup">\n<li><a href="<%=luci.dispatcher.build_url('"'"'admin_web'"'"','"'"'tonycn'"'"')%>">'"$NAME"'<\/a><\/li>/' /usr/lib/lua/luci/view/admin_web/network/index.htm
        fi
    fi
...
# 添加到手机版后台, modified from https://github.com/qiwihui/hiwifi-ss/blob/master/shadow.sh
    ...
    echo '<li> <a href="<%=luci.dispatcher.build_url('"'"'admin_web'"'"','"'"'tonycn'"'"')%>" target="_blank">科学上网<span class="right-bar"><em class="enter-pointer"></em></span></a> </li>' >> new_home.htm
